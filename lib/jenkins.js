...
function getViewJobs(viewName) {
  ...
  return request.get({ url, json: true })
    .then(result => Promise.map(filterExcludedJobs(result.jobs),
      job => getJobData(job.name, actionFilter), { concurrency: 1 }))
    .catch(error => ({ error }));
}
...
function filterExcludedJobs(jobs) {
  const exclusions = getViewExclusions();
  return jobs.filter(job => exclusions.indexOf(job.name) === -1);
}
...
function getViewExclusions() {
  return config.has('jenkins.excludes')
    ? config.get('jenkins.excludes')
    : [];
}
